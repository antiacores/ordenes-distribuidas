...
async def handle_event(message: aio_pika.IncomingMessage):
    async with message.process():
        ...
        if routing_key == "order.created":
            ...
            print(
                f"[analytics] Orden creada {order_id} | Total órdenes: {metrics['total_orders']} | Total items: {metrics['total_items']}"
            )
            ...
            print(
                f"[analytics] Orden confirmada {order_id} | Total confirmadas: {metrics['total_confirmed']}"
            )
            ...
            print(
                f"[analytics] Orden rechazada {order_id} | Total rechazadas: {metrics['total_rejected']}"
            )
