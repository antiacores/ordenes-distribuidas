...
async def signup_admin(
    body: dict, authorization: str = Header(...), payload: dict = Depends(verify_admin)
):
    return await forward_auth(
        "/auth/signup/admin", body, headers={"Authorization": authorization}
    )
