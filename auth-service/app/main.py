...
def signup_admin(body: AdminRegister, authorization: str = Header(...)):
    ...
    try:
        ...
        if payload.get("role") != "admin":
            raise HTTPException(status_code=403, detail="Solo un admin puede crear otros admins")
    except JWTError:
...
def validate_token(token: str):
    try:
        payload = jwt.decode(token, settings.jwt_secret, algorithms=["HS256"])
        return {"valid": True, "email": payload.get("sub"), "username": payload.get("username")}
    except JWTError:
        raise HTTPException(status_code=401, detail="Token inválido o expirado")